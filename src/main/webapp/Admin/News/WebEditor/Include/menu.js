...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	...
	s_MenuRow += ">"
	if (s_Image !=""){
		s_MenuRow += "&nbsp;<img border=0 src='sysimage/"+config.StyleDir+"/"+s_Image+"' width=20 height=20 align=absmiddle "+s_Disabled+">&nbsp;";
	}else{
		s_MenuRow += "&nbsp;";
	}
...
function showContextMenu(event){
	...
	if (isControlSelected("IMG")){
		sMenu += getMenuRow("", "ShowDialog('dialog/img.htm', 350, 335, true)", "img.gif", "图片属性...");
		sMenu += sMenuHr;
...
function showToolMenu(menu){
	...
	case "object":		// 对象效果菜单
		...
		break;
	case "component":	// 组件菜单
		sMenu += getMenuRow("", "ShowDialog('dialog/img.htm', 350, 335, true)", "img.gif", "插入或修改图片");
