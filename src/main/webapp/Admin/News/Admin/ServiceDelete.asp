...
Sub Content()
	...
	Case "check"
		If Not IsEmpty(strIdList) Then
			...
			For i = 0 To UBound(arrIdList)
				iId = Clng(arrIdList(i))			
				oConn.Execute("UPDATE Message SET Msgstate = 1 WHERE D_ID="&iId&"")			
			Next
	...
	Case "uncheck"
		If Not IsEmpty(strIdList) Then
			...
			For i = 0 To UBound(arrIdList)
				iId = Clng(arrIdList(i))			
				oConn.Execute("UPDATE Message SET Msgstate = 0 WHERE D_ID="&iId&"")
			Next
...
Sub DelNews(idstr)
	...
	sSql = "DELETE FROM message WHERE D_ID=" & idstr
	oConn.Execute sSql
