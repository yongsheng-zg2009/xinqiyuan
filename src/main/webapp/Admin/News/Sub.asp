...
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0"   style="word-break:break-all;">
  <%
dim lh, strelse
strelse=" and D_CataID<>'A00830001' and D_CataID<>'A00830002' "
lh=7
If News_Check = 1 Then
	If T_Special = "ok" Then		
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&" And D_Special =1  and D_Num = 1 ORDER BY D_SortID DESC"
	ElseIf T_Special = "hot" Then
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&"  and D_Num = 1 ORDER BY D_Hits DESC"
	Else	
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&"  and D_Num = 1 ORDER BY D_ID DESC"
	End If
Else
	If T_Special = "ok" Then		
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&"  And D_Special =1 ORDER BY D_SortID DESC"
	ElseIf T_Special = "hot" Then
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&"  ORDER BY D_Hits DESC"
	Else	
		sSql = "SELECT top "&Pmcount&" * FROM NewsData WHERE D_CataID LIKE '"&T_CataID&"%' "&strelse&"  ORDER BY D_ID DESC"
	End If
End if
  ...
  <tr align="center">
    <td valign="bottom">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
        <tr>
          <td height="22"><img src="/admin/Images/flag.gif" border="0" align="absmiddle" />&nbsp;<FONT COLOR="#000000">[<a href='InfoDir.asp?cataid=<%=oRs("D_cataid")%>' class='tt1'><FONT COLOR="#000000"><%call GetCataNameS1(ss_newsid)%></FONT></a>]</FONT> <a target="_blank" href="InfoView.asp?id=<%=oRs("D_id")%>&cataid=<%=oRs("D_CataID")%>" title="文章标题：<%=oRs("D_Title")%>&#13;发布时间：<%=oRs("D_AddTime")%>&#13;查看次数：<%=oRs("D_Hits")%>"><font color=<%=oRs("D_Title_color")%>><%=left(oRs("D_Title"),strlen)%></font></a>
		  <%
				if	(oRs("D_Icon")<>"") Then 
			%>
				<img src='<%=oRs("D_Icon")%>'>
...
%>
  <tr align="center">
    <td valign="bottom">
      <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" >
        <tr>
          <td height="18">·<a target="_blank" class="" href="NewsView.asp?id=<%=oRs("D_id")%>&cataid=<%=oRs("D_CataID")%>" title="文章标题：<%=oRs("D_Title")%>&#13;发布时间：<%=oRs("D_AddTime")%>&#13;查看次数：<%=oRs("D_Hits")%>"><font color=<%=oRs("D_Title_color")%>><%=left(oRs("D_Title"),strlen)%></font></a>
		  </td>
        </tr>
  ...
  <tr align="center">
    <td width="25%" valign="bottom">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" style="padding-left:10px">
        <tr>
          <td height="22"><img src="/Admin/Images/flag.jpg" align="absmiddle"> <a target="_blank" href="<%=UrlPage%>?id=<%=oRs("D_id")%>&cataid=<%=oRs("D_CataID")%>" title="文章标题：<%=oRs("D_Title")%>&#13发布时间：<%=oRs("D_AddTime")%>&#13查看次数：<%=oRs("D_Hits")%>" class="blackbig"><font color=<%=oRs("D_Title_color")%>><%=left(oRs("D_Title"),strlen)%></font></a>
			<%
				if	(oRs("D_Icon")<>"") Then 
			%>
				<img src='<%=oRs("D_Icon")%>'>
...
<% end sub %>
<% sub toplist_date_tz(T_CataID,Pmcount,strlen,T_Special,UrlPage)
%>
<table width="97%" border="0" cellpadding="0" cellspacing="0"   style="word-break:break-all;" valign="top">
  <%
dim lh
  ...
  <tr align="center">
    <td width="25%" valign="bottom">
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td height="22" style="line-height:150%;padding-top:3px;padding-bottom:3px"><img src="/admin/Images/flag.gif" align="absmiddle"> <a target="_blank" href="<%=UrlPage%>?id=<%=oRs("D_id")%>&cataid=<%=oRs("D_CataID")%>" title="文章标题：<%=oRs("D_Title")%>&#13发布时间：<%=oRs("D_AddTime")%>&#13查看次数：<%=oRs("D_Hits")%>" class="blackbig"><font color=<%=oRs("D_Title_color")%>><%=left(oRs("D_Title"),strlen)%></font></a>
			<%
				if	(oRs("D_Icon")<>"") Then 
			%>
				<img src='<%=oRs("D_Icon")%>'>
			<%
				End if
			%>
		  &nbsp;&nbsp;&nbsp;[<%=Month(FormatDateTime(oRs("D_AddTime"),2))&"."&Day(FormatDateTime(oRs("D_AddTime"),2))%>]</td>
        </tr>
		<tr>
          <td height="1" colspan="2"  background="/Admin/Images/jiange.gif"></td>
        </tr>
...
<%
	dim sqllink,Rslink
	sqllink = "SELECT top 20 * FROM L_Link where L_Hide=1 order by L_Sort"
	set Rslink=Server.CreateObject( "ADODB.Recordset" )
	Rslink.Open sqllink, oConn, 1, 1
	'Response.write "<TABLE cellSpacing=2 cellPadding=0 width='100%' align=center>"
	do while not Rslink.eof
	%>
	&nbsp;&middot;&nbsp;<a href="<%=Rslink("L_Url")%>" target="_blank"><%=Rslink("L_Name")%></a><BR>
...
%>
<% sub GetCataNameS1(s_ID)%>
<%
	dim sqllink,Rslink,S_CataID,s_color
	sqllink = "SELECT  D_CataID FROM NewsData WHERE D_ID = " & s_ID 
	set Rslink=Server.CreateObject( "ADODB.Recordset" )
	Rslink.Open sqllink, oConn, 1, 1
	do while not Rslink.eof
	...
	sqllink = "SELECT  left(C_Title,2) as C_Title FROM NewsCata WHERE C_RootID = '" & S_CataID &"'"
	If S_CataID="A00130001" Then
		s_color="#FF0000"
	End If 
	If S_CataID="A00130002" Then
		s_color="#336600"
	End if
	set Rslink=Server.CreateObject( "ADODB.Recordset" )
	Rslink.Open sqllink, oConn, 1, 1
	do while not Rslink.eof
		Response.write "<font color="&s_color&">"&Rslink("C_Title")&"</font>"
	Rslink.Movenext
...
%>
<% sub GetCataList(s_CataID,sinfolink)%>
<%
	dim Qrysql,Rslink,S_CataID_New,S_Img,i,sinfolink1
	...
	Response.write "<TABLE cellSpacing=2 cellPadding=0 width='100%' align=center style='background-color:#efefef'>"
	do while not Rslink.eof
		i=i+1
		if Len(Rslink("C_RootID"))=9 Then
			S_Img=""'"<IMG src=image/class.gif>"
		End If
		
		Response.write "<TR><TD height=22><a href="&sinfolink&"?CataID="&Rslink("C_RootID")&"><font style='FONT-SIZE: 12px; font-weight:bold;color:#ff0000'>"&Rslink("C_Title")&"</font></a></td></tr><TD height=2 background=image/index_hs_zh_32.jpg></TD></TR>"
		If i<>Rslink.RecordCount  Then
		End If
		Response.write "<TR><TD>"
			Call GetPageList(Rslink("C_RootID"),"ProductView.asp")
...
%>
<% sub GetCataList1(s_CataID,sinfolink)%>
<%
	dim Qrysql,Rslink,S_CataID_New,S_Img,i,sinfolink1
	...
	do while not Rslink.eof
		i=i+1
		if Len(Rslink("C_RootID"))=9 Then
			S_Img="<IMG src=image/index_hs_zh_35.jpg align=absMiddle>"
		End If
		
		Response.write "<TR><TD width=10 align=center valign=middle>"&S_Img&"</td><TD height=25><a href="&sinfolink&"?CataID="&Rslink("C_RootID")&"><font style='FONT-SIZE: 14px; '>"&Rslink("C_Title")&"</font></a></td></tr><TD height=1 colspan=2 background=image/index_hs_zh_56.jpg></TD></TR>"
		If i<>Rslink.RecordCount  Then
		End If
		Rslink.Movenext
	Loop
...
%>
<% sub GetPageList(s_CataID,sinfolink)%>
<%
	dim Qrysql,Rslink,S_CataID_New,S_Img,i
	
	S_Img="<IMG src=image/index_hs_zh_35.jpg align=absMiddle>"
	
	Qrysql = "SELECT  * FROM NewsData WHERE D_CataID = '" & S_CataID &"' order by D_SortID "
	set Rslink=Server.CreateObject( "ADODB.Recordset" )
	Rslink.Open Qrysql, oConn, 1, 1
	Response.write "<TABLE cellSpacing=2 cellPadding=0 width='100%' align=center style='background-color:#f5f5f5'>"
	do while not Rslink.eof

		Response.write "<TR><TD width=10 align=center valign=middle>"&S_Img&"</td><td><a href="&sinfolink&"?CataID="&Rslink("D_CataID")&"&ID="&Rslink("D_ID")&"><font style=' FONT-FAMILY: 宋体'>"&Rslink("D_Title")&"</font></a></tr><tr><td height=1 colspan=2 background=image/index_hs_zh_56.jpg style='padding-left:20px'></td></tr>"
...
<% sub GetPageList1(s_CataID,sinfolink)%>
<%
	dim Qrysql,Rslink,S_CataID_New,S_Img,i
	...
	do while not Rslink.eof
		...
		Response.write "<TR><TD width=10 align=center valign=middle height=23>"&S_Img&"</td><td><a href="&sinfolink&"?CataID="&Rslink("D_CataID")&"&ID="&Rslink("D_ID")&"><font style='FONT-SIZE: 14px; FONT-FAMILY: 宋体'>"&Rslink("D_Title")&"</font></a></tr><tr><td height=1 colspan=2 background=image/index_hs_zh_56.jpg style='padding-left:20px'></td></tr>"
...
<%
	...
	Qrysql = "SELECT  top 5 * FROM NewsCata WHERE C_RootID like '" & S_CataID &"%' and C_RootID <> '" & S_CataID &"' order by C_RootID"
	set Rslink=Server.CreateObject( "ADODB.Recordset" )
	Rslink.Open Qrysql, oConn, 1, 1
	do while not Rslink.eof
		i=i+1
		'if Len(Rslink("C_RootID"))=9 Then
		'	S_Img="&nbsp;<IMG  src=image/class3.gif align=absMiddle>"
		'End If
		'If Len(Rslink("C_RootID"))=13 Then
		'	S_Img="&nbsp;<IMG src=image/class4.gif align=absMiddle>"
		'End If
		Response.write S_Img&"<a href=NewsDir.asp?CataID="&Rslink("C_RootID")&">"&Rslink("C_Title")&"</a>&nbsp;|&nbsp;"
		'If i<>Rslink.RecordCount  Then
		'Response.write "<BR>"
		'End if
	Rslink.Movenext
